...
const buildRoot = path.resolve(root, ".tmp", "vector-provider-evaluation-build");
const disposableRoot = `${path.resolve(root, ".tmp")}${path.sep}`;
if (!buildRoot.startsWith(disposableRoot)) {
  throw new Error("Refusing to clean an evaluation build directory outside workspace .tmp.");
}
...
const cli = path.join(buildRoot, "scripts", "retrieval", "vector-provider-evaluation-cli.js");
